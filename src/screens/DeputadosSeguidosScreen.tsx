...
export function DeputadosSeguidosScreen() {
  ...
  const { mutate: unfollow } = useMutation({
    ...
    onSuccess: (data, politicianId) => {
      ...
      void queryClient.invalidateQueries({ queryKey: ['isFollowing', politicianId] });
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        <View style={styles.headerTop}>
          <Text style={styles.title}>Deputados Seguidos</Text>
          <TouchableOpacity
          ...
          renderItem={({ item }) => (
            ...
            >
              <DeputiesCard
                ...
                isFollowed={true} // Deputies in this list are always followed
                expenseTotal={item.expenseTotal ?? 0}
                propositionsTotal={item.propositionsTotal ?? 0}
                onToggleFollow={() => unfollow(item.id)}
              />
            </TouchableOpacity>
          )}
          refreshControl={
